...
int main() {
  ...
  for (int m = 0; m < num_matrices; m++) {
    ...
    // thread till all the HIP operations in the stream1 until the
    // hipEventRecord are done. Also note that it is actually stream2 that will
    // wait on the datatransfer event (which we place in stream1). So stream2
    ...
    // event at the point marked by hipEventRecord is reached.
